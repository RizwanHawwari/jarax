<x-admin-layout>
    ...
    <form action="{{ route('admin.staff.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-4xl">
        @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="staff_status" required
                        ...
                        <option value="active" {{ old('staff_status') == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="cuti" {{ old('staff_status') == 'cuti' ? 'selected' : '' }}>Cuti</option>
                        <option value="inactive" {{ old('staff_status') == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                    @error('staff_status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F95738] @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
